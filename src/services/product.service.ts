...
class ProductService {
    ...
    public async createProductService(product: IProduct): Promise<IProductDocument> {
        try {
            if (!product.product_name)
                throw new ExpectationFailedError(`Please enter product name`);
            if (!product.product_quantity)
                throw new ExpectationFailedError(`Please enter product code`);
            if (!product.product_category_id)
                throw new ExpectationFailedError(`Please enter product category`);
            if (!product.product_unit)
                throw new ExpectationFailedError(`Please enter product price`)
    ...
    public async getProductService(product_id: Types.ObjectId): Promise<IProductDocument | null> {
        try {
            if (!product_id)
                throw new ExpectationFailedError(`Please enter product id`);
    ...
    public async createProductCategoryService(product_category: IProductCategory): Promise<IProductCategoryDocument> {
        try {
            if (!product_category.product_category_name)
                throw new ExpectationFailedError(`Please enter product category name`);
    ...
    public async getProductCategoryService(product_category_id: Types.ObjectId): Promise<IProductCategoryDocument | null> {
        try {
            if (!product_category_id)
                throw new ExpectationFailedError(`Please enter product category id`);
