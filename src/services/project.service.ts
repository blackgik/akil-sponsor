import { BadRequestError, ExpectationFailedError } from '../utils/app.errors';
...
class ProjectService {
    ...
    public async createProjectService(project: IProject): Promise<IProjectDocument> {
        try {
            if (!project.project_business_name)
                throw new ExpectationFailedError(`Please enter project name`);
            if (!project.project_code)
                throw new ExpectationFailedError(`Please enter project code`);
    ...
    public async getProjectService(project_id: Types.ObjectId): Promise<IProjectDocument | null> {
        try {
            if (!project_id)
                throw new ExpectationFailedError(`Please enter project id`);
