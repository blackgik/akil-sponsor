import { BadRequestError, ExpectationFailedError } from '../utils/exceptions/http.exception';
...
class ProjectService {
    ...
    public async createProjectService(project: IProject): Promise<IProjectDocument> {
        try {
            if (!project.project_business_name)
                throw new ExpectationFailedError(400,`Please enter project name`,'');
            if (!project.project_code)
                throw new ExpectationFailedError(400,`Please enter project code`,'');
    ...
    public async getProjectService(project_id: Types.ObjectId): Promise<IProjectDocument | null> {
        try {
            if (!project_id)
                throw new ExpectationFailedError(400,`Please enter project id`,'400');
    ...
    public async updateProjectService(project_id: Types.ObjectId, project: IProject): Promise<IProjectDocument | null> {
        try {
            if (!project_id)
                throw new ExpectationFailedError(400,`Please enter project id`,'');
